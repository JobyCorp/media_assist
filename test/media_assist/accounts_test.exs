defmodule MediaAssist.AccountsTest do
  use MediaAssist.DataCase

  alias MediaAssist.Accounts

  import MediaAssist.AccountsFixtures
  alias MediaAssist.Accounts.{User, UserToken}
  ...
  describe "register_user/1" do
    ...
    test "registers a user with a hashed password, confirmed immediately" do
  ...
  describe "update_user_email/2" do
    ...
    test "rejects an invalid email" do
      ...
      assert {:error, changeset} = Accounts.update_user_email(user, %{email: "not valid"})
    ...
    test "rejects a taken email" do
      ...
      assert {:error, changeset} = Accounts.update_user_email(user, %{email: taken})
      assert "has already been taken" in errors_on(changeset).email
    end
    ...
    test "requires the email to change" do
      ...
      assert {:error, changeset} = Accounts.update_user_email(user, %{email: user.email})
